        ...
        {EDU.map((e, i) => (
          ...
          >
            <div className="mx-auto grid h-16 w-16 place-items-center rounded-full border border-[color:var(--neon)] animate-pulse-glow">
              {i === 0 ? <Trophy className="h-7 w-7 text-neon" /> : <GraduationCap className="h-7 w-7 text-neon" />}
            </div>
            <div className="mt-4 font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Milestone</div>
            <h3 className="mt-2 text-xl">{e.deg}</h3>
            <p className="mt-1 text-sm text-muted-foreground">{e.school} · {e.year}</p>
          </motion.div>
        ))}
      </div>
    </Section>
